# src/ui/app.py  — Final v2
# Fixes: per-user portfolio, compact UI, sidebar toggle, date fix, stronger agents
...
button[data-testid="collapsedControl"]{
    ...
    box-shadow:2px 0 12px rgba(99,102,241,0.2) !important;
}
...
/* ── NEW: Groww/INDMoney-style pick cards ── */
.pk{background:rgba(255,255,255,0.022);border:1px solid rgba(255,255,255,0.06);border-radius:12px;overflow:hidden;margin-bottom:10px;transition:border-color .2s;}
.pk:hover{border-color:rgba(255,255,255,0.12);}
.pk-top{padding:14px 16px 10px;border-left:3px solid transparent;}
.pk-top.bull{border-left-color:#10b981;}.pk-top.bear{border-left-color:#ef4444;}.pk-top.neut{border-left-color:#f59e0b;}
.pk-sym{font-family:'IBM Plex Mono',monospace;font-size:16px;font-weight:700;color:#e2e8f0;}
.pk-co{font-size:11px;color:#475569;margin-top:1px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.pk-price{font-family:'IBM Plex Mono',monospace;font-size:16px;font-weight:600;color:#e2e8f0;text-align:right;}
.pk-meta-row{display:flex;align-items:center;gap:6px;margin-top:8px;flex-wrap:wrap;}
.pk-bar{background:rgba(255,255,255,0.03);border-top:1px solid rgba(255,255,255,0.05);padding:8px 16px;display:flex;gap:16px;font-size:11px;color:#64748b;}
.pk-stat{display:flex;flex-direction:column;gap:2px;}
.pk-stat-lbl{font-size:10px;color:#475569;text-transform:uppercase;letter-spacing:.05em;}
.pk-stat-val{font-family:'IBM Plex Mono',monospace;font-size:12px;font-weight:500;color:#94a3b8;}
.badge{display:inline-block;padding:2px 7px;border-radius:4px;font-size:11px;font-weight:500;margin:1px;}
...
/* ── NEW: Compact AI analysis cards ── */
.ai-card{border-radius:10px;padding:12px 14px;margin-bottom:8px;display:flex;align-items:flex-start;gap:12px;}
.ai-action-pill{min-width:70px;padding:6px 10px;border-radius:8px;text-align:center;font-size:12px;font-weight:700;letter-spacing:.03em;flex-shrink:0;}
.ai-body{flex:1;min-width:0;}
.ai-ticker{font-family:'IBM Plex Mono',monospace;font-weight:700;font-size:13px;color:#e2e8f0;}
.ai-summary{font-size:12.5px;color:#94a3b8;margin-top:3px;line-height:1.4;}
.ai-levels{display:flex;gap:16px;margin-top:8px;}
.ai-level{font-size:11px;}
.ai-level-lbl{color:#475569;}
.ai-level-val{font-family:'IBM Plex Mono',monospace;font-weight:500;}
...
ACTION_STYLE = {
    "HOLD":     ("HOLD",   "#f59e0b","rgba(245,158,11,.15)"),
    "SELL":     ("SELL",   "#ef4444","rgba(239,68,68,.15)"),
    "BUY_MORE": ("BUY ↑",  "#10b981","rgba(16,185,129,.15)"),
}
...
def can_run_analysis() -> bool:
    ...
    runs     = sess.query(ApiUsage).filter(
        ApiUsage.user_id==CURRENT_UID, ApiUsage.timestamp>=week_ago).count()
    sess.close()
    return runs < 3
...
def run_denied_msg():
    if ACCOUNT_TYPE == "guest":
        return "👁 Guest accounts cannot run analysis. Create a free account."
...
def get_user_open_tickers(uid:str)->set:
    """Returns tickers THIS user has open. Strictly per-user — no null fallback."""
    if not uid or uid=="guest":return set()
    sess=Session()
    rows=sess.query(Portfolio).filter(
        Portfolio.is_open==True,
        or_(Portfolio.user_id==uid, Portfolio.username==uid)
    ).all()
    sess.close()
...
def render_mbar():
    s=market_status()
    def pill(info):
        cls="mk-open" if info["open"] else "mk-shut";dot="on" if info["open"] else "off"
...
def render_pick_card(p, prefix="", user_tickers:set=None):
    """Compact Groww/INDMoney-style pick card."""
    ...
    # Key stats row
    why_short  = (p.why_buy[0][:60]+"…") if p.why_buy else "—"
    risk_short = (p.why_not_buy[0][:50]+"…") if p.why_not_buy else "—"
      ...
      <div class="pk-top {cls}">
        <div style="display:flex;justify-content:space-between;align-items:flex-start">
          <div style="flex:1;min-width:0">
            ...
            <div class="pk-co">{p.company[:40]}</div>
          </div>
          <div style="text-align:right;flex-shrink:0;margin-left:12px">
            <div class="pk-price">{cur}{p.price_at_pick:,.2f}</div>
            <div style="margin-top:5px">{sbadge(p.technical_signal)}{sbadge(p.sentiment)}{cbadge(p.confidence)}</div>
          </div>
        </div>
        <div style="margin-top:10px;font-size:12px;color:#64748b">{p.sector} &nbsp;·&nbsp; {p.size} Cap &nbsp;·&nbsp; {p.analysis_date}</div>
      </div>
      <div class="pk-bar">
        <div class="pk-stat">
          <span class="pk-stat-lbl">Why Buy</span>
          <span class="pk-stat-val" style="color:#94a3b8;font-family:'Outfit',sans-serif;font-size:11.5px">{why_short}</span>
        </div>
      </div>
    ...
    with c_add:
        if ACCOUNT_TYPE=="guest":
            ...
            st.markdown('<div style="text-align:center;padding:8px 0;font-size:18px;color:#10b981">✓</div>',
                        unsafe_allow_html=True)
            st.caption("Added")
        else:
...
def render_analysis_card(res:dict):
    """Compact single-line analysis card — Groww style."""
    ...
    try:
        ...
        sl_v=round(float(sl),2) if sl and str(sl).lower() not in ("null","none","") else None
        tp_v=round(float(tp),2) if tp and str(tp).lower() not in ("null","none","") else None
    except Exception:sl_v=tp_v=None

    ticker_sym=res.get("ticker","").replace(".NS","").replace(".BO","")
    reasons_html="".join(f"<div style='font-size:12px;color:#64748b;margin-top:3px'>• {r}</div>" for r in reasons)

    levels=""
    if sl_v:levels+=f'<div class="ai-level"><span class="ai-level-lbl">Stop </span><span class="ai-level-val" style="color:#ef4444">{sl_v:,.2f}</span></div>'
    if tp_v:levels+=f'<div class="ai-level"><span class="ai-level-lbl">Target </span><span class="ai-level-val" style="color:#10b981">{tp_v:,.2f}</span></div>'
    ...
    <div class="ai-card" style="background:{bg};border:1px solid {color}22;">
      ...
      <div class="ai-body">
        <div style="display:flex;justify-content:space-between;align-items:center">
          <span class="ai-ticker">{ticker_sym}</span>
          <span style="font-size:11px;color:#475569">{conf} confidence</span>
        </div>
...
def render_ptab(market=None,pfx="all"):
    ...
    if pfx=="all":
        ...
        with tr:
            tog=st.radio("",["₹ INR","$ USD"],horizontal=True,key="curr_tog",label_visibility="collapsed")
            display_inr=(tog=="₹ INR")
    ...
    if h:
        ...
        for x in h:
            ...
            if pfx=="all":
                if display_inr and dx["currency"]=="USD":
                    for k in ("entry_price","current_price","invested","current_value","pnl_amount"):dx[k]*=rate
                    dx["dcur"]="₹"
                elif not display_inr and dx["currency"]=="INR":
                    for k in ("entry_price","current_price","invested","current_value","pnl_amount"):dx[k]/=rate
                    dx["dcur"]="$"
                else:dx["dcur"]="₹" if dx["currency"]=="INR" else "$"
            else:dx["dcur"]="₹" if market=="INDIA" else "$"
            disp.append(dx)

        inv=sum(x["invested"] for x in disp);val=sum(x["current_value"] for x in disp)
        pnl=val-inv;pct=pnl/inv*100 if inv else 0
        pc="c-up" if pnl>=0 else "c-dn";cur=disp[0]["dcur"] if disp else ""
        st.markdown(f"""
        ...
        # Portfolio table with AI analysis column
        rows=[]
        for x in disp:
            c=x["dcur"]
            ai_action=x.get("ai_action") or "—"
            ai_color ={"HOLD":"#f59e0b","SELL":"#ef4444","BUY_MORE":"#10b981"}.get(ai_action,"#64748b")
            rows.append({
                "Ticker":  x["ticker"].replace(".NS","").replace(".BO",""),
                "Company": x["company"][:20],
                "Market":  x["market"],
        ...
        st.dataframe(
            df.style
              ...
              .map(lambda v:f"color:{'#10b981' if v=='BUY_MORE' else '#ef4444' if v=='SELL' else '#f59e0b' if v=='HOLD' else '#64748b'}",subset=["AI"])
              .format({"P&L %":"{:+.2f}%"}).hide(axis="index"),
    ...
    # AI Analysis
    st.markdown('<div class="sdiv">🤖 AI Analysis</div>',unsafe_allow_html=True)
    if not h:
        ...
        with ci:
            st.markdown('<div style="font-size:12px;color:#475569;padding-top:8px">Claude Haiku · ~5s/stock · Results saved to portfolio</div>',
                        unsafe_allow_html=True)
        if run_ai:
        ...
        if f"ai_results_{pfx}" in st.session_state:
            ...
            if isinstance(results,list) and results:
                ...
                <div style="display:flex;gap:8px;margin-bottom:12px">
                  <span style="background:rgba(16,185,129,.1);border:1px solid rgba(16,185,129,.2);color:#10b981;padding:3px 12px;border-radius:6px;font-size:12px">Buy More: {acts.count("BUY_MORE")}</span>
                  <span style="background:rgba(245,158,11,.1);border:1px solid rgba(245,158,11,.2);color:#f59e0b;padding:3px 12px;border-radius:6px;font-size:12px">Hold: {acts.count("HOLD")}</span>
                  <span style="background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.2);color:#ef4444;padding:3px 12px;border-radius:6px;font-size:12px">Sell: {acts.count("SELL")}</span>
                </div>""",unsafe_allow_html=True)
                for res in results:render_analysis_card(res)

    # Add Position
    st.markdown('<div class="sdiv">➕ Add Position</div>',unsafe_allow_html=True)
    st.markdown('<div style="font-size:12px;color:#475569;margin-bottom:12px">Enter your actual buy price, or leave as 0 for live price.</div>',unsafe_allow_html=True)
    with st.form(key=f"add_{pfx}",clear_on_submit=True):
        ...
        if st.form_submit_button("➕ Add to Portfolio",use_container_width=True):
            if not new_ticker.strip():st.error("Ticker is required.")
            else:
                try:
                    add_to_portfolio(ticker=new_ticker.strip().upper(),quantity=new_qty,
                                     user_id=CURRENT_UID,username=CURRENT_USER,
    ...
    if h:
        ...
        with st.form(key=f"sell_{pfx}",clear_on_submit=True):
            opts=[f"{x['id']}  ·  {x['ticker']}  ·  {int(x['quantity'])} shares  ·  {x['pnl_pct']:+.2f}%" for x in h]
...
with st.sidebar:
    ...
    if ACCOUNT_TYPE=="admin":
        nav_opts.insert(1,"Admin");nav_icons.insert(1,"shield-fill")

    page=option_menu(menu_title=None,options=nav_opts,icons=nav_icons,default_index=0,
        styles={"container":{"background":"transparent","padding":"0 8px"},
                "menu-icon":{"display":"none"},
                "nav-link":{"font-size":"13px","color":"#64748b","font-family":"'Outfit',sans-serif",
                            "padding":"9px 14px","border-radius":"8px","margin":"2px 0",
                            "--hover-color":"rgba(255,255,255,0.04)"},
                "nav-link-selected":{"background":"rgba(99,102,241,0.12)","color":"#818cf8","font-weight":"500"},
                "icon":{"font-size":"14px"}})
    ...
    rem=runs_remaining()
    if rem is not None:
        pct_bar=rem/3;bar_color="#10b981" if pct_bar>0.5 else "#f59e0b" if pct_bar>0 else "#ef4444"
        st.markdown(f"""
    ...
    cfg=load_sched();dc="#10b981" if cfg["enabled"] else "#ef4444";dl="Active" if cfg["enabled"] else "Off"
    nr=(f'<div style="color:#6366f1;font-size:11px;margin-top:5px">Every {cfg["day"].title()} at {cfg["hour"]:02d}:{cfg["minute"]:02d}</div>' if cfg["enabled"] else "")
    st.markdown(f"""
    <div style="padding:0 12px;margin-bottom:10px">
...
if page=="Dashboard":
    ...
    if recent:
        ...
        for i,p in enumerate(recent):
            with(c1 if i%2==0 else c2):render_pick_card(p,"dash",user_tickers)
    else:
        st.markdown('<div style="color:#334155;text-align:center;padding:4rem 0;font-size:14px">No picks yet — click ▶ Run in the sidebar</div>',unsafe_allow_html=True)

elif page=="Admin":
    if ACCOUNT_TYPE!="admin":st.error("Access denied.");st.stop()
    render_mbar()
    ...
    st.markdown('<div class="sdiv">Indian Market Picks &nbsp;·&nbsp; NSE / BSE</div>',unsafe_allow_html=True)
    secs=sectors_db("INDIA");szs=sizes_db("INDIA")
    if not secs:
        st.markdown('<div style="color:#334155;text-align:center;padding:4rem 0">No India picks yet.</div>',unsafe_allow_html=True)
    else:
        c1,c2=st.columns(2)
        ...
        for i,p in enumerate(picks):
            with(ca if i%2==0 else cb):render_pick_card(p,"in",user_tickers)

elif page=="US Markets":
    render_ticker("us");render_mbar();guest_banner()
    st.markdown('<div class="sdiv">US Market Picks &nbsp;·&nbsp; NYSE / NASDAQ</div>',unsafe_allow_html=True)
    secs=sectors_db("US");szs=sizes_db("US")
    if not secs:
        st.markdown('<div style="color:#334155;text-align:center;padding:4rem 0">No US picks yet.</div>',unsafe_allow_html=True)
    else:
        c1,c2=st.columns(2)
        ...
        ca,cb=st.columns(2)
        for i,p in enumerate(picks):
            with(ca if i%2==0 else cb):render_pick_card(p,"us",user_tickers)
    ...
    m=cached_metrics(uid=CURRENT_UID)
    bp=m.get("best_pick");bpp=m.get("best_pick_pct");wp=m.get("worst_pick");wpp=m.get("worst_pick_pct");wr=m.get("win_rate")
    st.markdown(f"""
    <div class="metric-strip">
      <div class="mc"><div class="mc-lbl">Win Rate</div><div class="mc-val">{f"{wr}%" if wr else "—"}</div><div class="mc-sub c-mu">{m.get('wins',0)} W · {m.get('losses',0)} L</div></div>
    ...
    if ACCOUNT_TYPE!="guest":
        ...
        my=get_usage_stats(user_id=CURRENT_UID)
        c1,c2,c3=st.columns(3)
    ...
    if ACCOUNT_TYPE in ("guest","free"):
        ...
        with c1:
            ...
            enabled=st.toggle("Enable",cfg.get("enabled",False))
            day=st.selectbox("Day",["mon","tue","wed","thu","fri"],
                             index=["mon","tue","wed","thu","fri"].index(cfg.get("day","mon")))
            ch,cm=st.columns(2)
            hour=ch.number_input("Hour",0,23,cfg.get("hour",6))
            minute=cm.selectbox("Minute",[0,15,30,45],index=[0,15,30,45].index(cfg.get("minute",0)))
        with c2:
            st.markdown("**Scope**")
        ...
        if st.button("💾 Save",type="primary"):
            ...
            if enabled:
                ...
                except Exception as e:st.warning(f"Saved (scheduler error: {e})")
            else:st.success("Saved · Scheduler off")
    ...
    if ACCOUNT_TYPE!="guest":
        ...
        with st.form("chpwd",clear_on_submit=True):
            np1=st.text_input("New password",type="password",placeholder="Min 6 chars",label_visibility="collapsed")
            np2=st.text_input("Confirm",type="password",placeholder="Confirm password",label_visibility="collapsed")
