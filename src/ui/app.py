...
.pk-co{font-size:11px;color:#475569;margin-top:1px;}
.pk-price{font-family:'IBM Plex Mono',monospace;font-size:16px;font-weight:600;color:#e2e8f0;text-align:right;}
.pk-bar{background:rgba(255,255,255,0.02);border-top:1px solid rgba(255,255,255,0.05);padding:8px 16px;font-size:11.5px;color:#64748b;line-height:1.5;}
...
def guest_banner():
    if ACCOUNT_TYPE=="guest":
        st.markdown('<div class="guest-banner">👁 Guest mode — read only. Create a free account to add picks.</div>',
                    unsafe_allow_html=True)
...
def render_pick_card(p, prefix="", user_tickers:set=None):
    ...
    with c_view:
        if st.button("View Details →", key=f"v_{prefix}_{p.id}", use_container_width=True):
            st.session_state.selected_stock    = p.ticker
            st.session_state.selected_pick_id  = p.id
            st.rerun()
...
def render_ptab(market=None,pfx="all"):
    ...
    if h:
        ...
        for x in disp:
            c=x["dcur"]
            rows.append({"Ticker":x["ticker"].replace(".NS","").replace(".BO",""),
                         "Company":x["company"][:20],"Market":x["market"],
                         "Entry":f"{c}{x['entry_price']:,.2f}","Current":f"{c}{x['current_price']:,.2f}",
                         "Qty":int(x["quantity"]),"P&L %":x["pnl_pct"],
                         "P&L Amt":f"{c}{x['pnl_amount']:+,.2f}",
                         "AI":x.get("ai_action") or "—","Days":x["days_held"]})
        ...
        st.markdown('<div style="color:#334155;text-align:center;padding:2rem 0;font-size:14px">No open positions</div>',unsafe_allow_html=True)

    if ACCOUNT_TYPE=="guest":
        st.markdown('<div class="guest-banner">👁 Sign in to add positions and get AI analysis.</div>',unsafe_allow_html=True)
        return
...
with st.sidebar:
    ...
    rem=runs_remaining()
    if rem is not None:
        pct_bar=rem/3;bc="#10b981" if pct_bar>0.5 else "#f59e0b" if pct_bar>0 else "#ef4444"
        st.markdown(f"""
        <div style="padding:0 12px;margin-bottom:10px">
          ...
          <div style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:8px;padding:10px 12px">
            <span style="font-size:13px;color:{bc};font-weight:500">{rem} / 3 remaining</span>
            <div style="height:3px;background:rgba(255,255,255,0.05);border-radius:2px;margin-top:8px;overflow:hidden">
              <div style="height:100%;width:{int(pct_bar*100)}%;background:{bc};border-radius:2px"></div>
            </div>
          </div>
        </div>""",unsafe_allow_html=True)
...
if page=="Dashboard":
    ...
    if not secs:
        ...
        c1,c2=st.columns(2)
        ss=c1.selectbox("Sector",["All"]+secs,key="in_sec");sz=c2.selectbox("Size",["All"]+szs,key="in_siz")
    ...
    if not secs:
        ...
        c1,c2=st.columns(2)
        ss=c1.selectbox("Sector",["All"]+secs,key="us_sec");sz=c2.selectbox("Size",["All"]+szs,key="us_siz")
        ...
        if st.button("💾 Save",type="primary"):
            ...
            if enabled:
                try:
                    ...
                    st.success(f"✅ ON — every {day.title()} at {int(hour):02d}:{int(minute):02d}")
