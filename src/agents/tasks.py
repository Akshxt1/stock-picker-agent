...
def create_tasks(market: str, sector: str, size: str):
    ...
    task_research = Task(
        description=(
            ...
            f"  - Debt-to-equity ratio > 3.0 (HIGH RISK) — skip this check if Debt/Eq is N/A\n"
            f"  - Revenue growth < 0% (DECLINING) — skip this check if RevGrowth is N/A\n"
            f"  - 30-day momentum < -15% (SEVERE DOWNTREND)\n"
            f"  - ROE < 8% (POOR QUALITY) — skip this check if ROE is N/A\n"
            f"  NOTE: If Market Cap is Unknown/N/A, classify the stock as '{size} Cap' "
            f"per the user's selection — do NOT drop a stock just because market cap data is missing.\n\n"
